    ...
    <div className="min-h-screen bg-gradient-to-br from-violet-950/30 via-background to-background">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
          <div>
            <h1 className="text-3xl font-bold mb-2">Gestión de Ofertas</h1>
            <p className="text-muted-foreground">
              Cree y gestione sus ofertas de empleo
            </p>
          </div>
          <Link
            href="/es/compte/dashboard/recruteur/offres/nouvelle"
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          ...
          <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-4">
            <p className="text-sm text-emerald-400">Activas</p>
            <p className="text-2xl font-bold text-emerald-400">{offresActives.length}</p>
          </div>
          <div className="rounded-xl border border-red-500/30 bg-red-500/10 p-4">
            <p className="text-sm text-red-400">Expiradas</p>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 rounded-xl border border-border bg-background/50">
            ...
            <p className="text-muted-foreground mb-4">Ninguna oferta encontrada</p>
            <Link
              href="/es/compte/dashboard/recruteur/offres/nouvelle"
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Creada el {new Date(offre.createdAt).toLocaleDateString('es-ES')}
                      {offre.expiresAt && (
                        <> • Expira el {new Date(offre.expiresAt).toLocaleDateString('es-ES')}</>
                      )}
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    <Link
                      href={`/es/compte/dashboard/recruteur/offres/${offre.id}`}
                    ...
                    <Link
                      href={`/es/compte/dashboard/recruteur/offres/${offre.id}/modifier`}
                      className="p-2 rounded-lg hover:bg-violet-500/10 text-violet-400"
                      title="Modificar"
                    >
