...
export default function AdminRecruitersPage() {
  ...
  const checkAdminRole = async () => {
    ...
    if (role !== 'admin') {
      router.push('/en/compte/dashboard');
      return;
    }
    fetchRecruiters();
  };
  ...
  const fetchRecruiters = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const result = await response.json();
        throw new Error(result.error || 'Error during retrieval');
      }
    } catch (error) {
  ...
  const handleAction = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const result = await response.json();
        throw new Error(result.error || 'Error during validation');
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-violet-950/30 via-background to-background">
      ...
      <main className="container mx-auto px-4 py-8">
        {/* Back button */}
        <Link
          href="/en/compte/dashboard/admin"
        ...
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">
            Validate or reject recruiter account requests
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 rounded-xl border border-border bg-background/50">
            ...
            <p className="text-muted-foreground mb-4">No recruiters found</p>
          </div>
        ) : (
                ...
                <div className="flex items-center justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-muted-foreground">
                      Registered on {new Date(recruiter.createdAt).toLocaleDateString('en-US')}
                      {recruiter.validatedAt && (
                        <span className="ml-4">
                          • Validated on {new Date(recruiter.validatedAt).toLocaleDateString('en-US')}
                        </span>
                      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-xl bg-background border border-border p-6 shadow-xl">
            <h3 className="text-xl font-bold mb-4">
              {modalAction === 'validate' && 'Validate this recruiter?'}
              {modalAction === 'reject' && 'Reject this recruiter?'}
              {modalAction === 'details' && 'Recruiter details'}
            </h3>
            ...
            <div className="space-y-3 mb-6">
              ...
              <div className="flex justify-between">
                ...
                <span className={
                  ...
                  {selectedRecruiter.isValidated ? 'Validated' :
                   selectedRecruiter.rejectedAt ? 'Rejected' : 'Pending'}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Registration:</span>
                <span>{new Date(selectedRecruiter.createdAt).toLocaleDateString('en-US')}</span>
              </div>
